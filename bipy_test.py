...
class BiPyTests(unittest.TestCase):
	...
	def test__get_parameters(self):
		sut = bipy.BiPy('key', 'secret')
		self.assertEqual(sut._BiPy__get_parameters(symbol = 'TRXBTC', timestamp=1550964774431), self.RESULT_PARAMETER_LIST)
	...
	def test__get_uri_when_signed(self):
		sut = bipy.BiPy('key', 'secret')
		self.assertRegexpMatches(sut._BiPy__get_uri(bipy.BiPy.ENDPOINT_ALL_ORDERS, True, symbol = 'TRXBTC'), self.REGEX_GET_URI_SIGNED)
		
	def test__get_uri_when_unsigned(self):
			sut = bipy.BiPy('key', 'secret')
			self.assertEqual(sut._BiPy__get_uri(bipy.BiPy.ENDPOINT_DEPTH, False, symbol = 'TRXBTC', limit=20), self.RESULT_GET_URI_UNSIGNED)
			
	def test__get_uri_when_unsigned_no_paramters(self):
			sut = bipy.BiPy('key', 'secret')
			self.assertEqual(sut._BiPy__get_uri(bipy.BiPy.ENDPOINT_PRICE, False), self.RESULT_GET_URI_UNSIGNED_NO_PARAMETERS)
			
	#def test_ping_time(self):
	#	sut = bipy.BiPy('key', 'secret')
	#	self.assertRegexpMatches(sut.ping_time()), self.REGEX_GET_URI_SIGNED)	
